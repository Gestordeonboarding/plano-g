...
// Usado no cadastrar — retorna 409 se já tem user_id (já cadastrado)
export async function POST(request: Request) {
  try {
    ...
    if (c.user_id) return NextResponse.json({ error: 'Já cadastrado.' }, { status: 409 })

    return NextResponse.json({ name: c.full_name })
  } catch (err) {
